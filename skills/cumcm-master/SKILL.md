...
首次在一个建模工作区使用时，先定位插件根目录下的
`automcm-pro-runtime/bootstrap_runtime.py` 并运行：

```bash
python "<插件根目录>/automcm-pro-runtime/bootstrap_runtime.py" --target .
```

它会准备本 skill 所需的 `scripts/`、`templates/` 和 `AutoMCM_SOP.md`，默认不覆盖工作区
中的同名文件。随后再执行下文的 `python scripts/...` 命令。
