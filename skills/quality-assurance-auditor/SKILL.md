...
- `paper/figures/figure_manifest.json` and `paper/figures/figure_qa_log.md` from `modeling-figure-orchestrator`
...
- Optional `paper/reviews/modeling-panel/panel_decision.md` and `revision_roadmap.md` from `academic-paper-reviewer`.
...
1. Check problem coverage.
   - If a modeling-panel review exists, verify every `BLOCKING` or `MAJOR` item is resolved or explicitly inherited as a QA blocker.
   - The panel is advisory; its `READY_FOR_G6` decision cannot replace any sibling audit.
...
7. Check paper quality.
   ...
   - Verify the four required evidence families (model principle, key comparison, sensitivity, and
     decision scheme) are covered or have a defensible N/A; reject decorative and mergeable
     low-density figures.
   - Verify key values, extrema, uncertainty/error bars or confidence intervals, and baselines are
     present or carry a source-grounded `N/A: reason`; do not require fabricated uncertainty.
   ...
   - Audit the abstract: one paragraph has 3-5 semantic bold groups covering method, model, metric,
     conclusion value, and ending keywords; bold spans contain keywords only, never full sentences.
