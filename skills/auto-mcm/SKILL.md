...
本 skill 已打包进 `mathmodeling-skills` 插件。首次在一个建模工作区使用时，先根据当前
`SKILL.md` 的位置定位插件根目录下的 `automcm-pro-runtime/bootstrap_runtime.py`，运行：

```bash
python "<插件根目录>/automcm-pro-runtime/bootstrap_runtime.py" --target .
```

该命令会把所需的 `scripts/`、`templates/` 和 `AutoMCM_SOP.md` 复制到当前工作区，且默认
保留已有文件。完成后再执行下文中的 `python scripts/...` 命令。
