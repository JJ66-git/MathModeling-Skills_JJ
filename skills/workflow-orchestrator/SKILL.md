...
- **pass_criteria**:
  ...
  - For CUMCM/全国赛, the paper contains no generated table of contents unless the official template explicitly requires one; `2_analysis` contains a whole-problem overview plus one evidence-based, itemized analysis block for every parsed subquestion.
  - The required-figure matrix includes one paper-overview schematic. Every important algorithm has both an algorithm flowchart and a concept/mechanism diagram, with editable Draw.io source, final export, clear non-crossing routes, and `PASS` QA before G5 can pass.
...
problem-parser → problem-classifier → deep-research (optional) → related-paper-analyzer
...
figure-table-planner → paper/figures/ planning
    ↓
【10】图表生成与视觉验收
modeling-figure-orchestrator → figure_manifest.json + figure_qa_log.md
    ↓
【11】论文写作 (Per-Question)
paper-section-writer → paper/sections/qx.tex
    ↓
【12】建模手审核 → 【13】编程手审核
    ↓
【14】独立审计层 (NEW — three orthogonal audits, all must PASS to advance)
...
3. `deep-research` (optional when the task needs web literature retrieval, method comparison, or fact-checking)
4. `related-paper-analyzer` (canonical literature-to-modeling handoff; consume verified deep-research outputs if present)
5. **Create global symbol table, model assumptions, dependency map** (planning stage)
6. `method-selector` (per Qx — can run in parallel for independent subquestions)
7. `model-code-analyzer`
8. `python-model-code-generator` or `matlab-model-code-generator`
9. `code-reviewer`
10. `result-report-generator` (round N experiment report)
11. **Loop back to 6 or 7** if method revision needed
12. `result-report-generator` (final result analysis, when method is locked)
13. `final-method-explainer`
14. `robustness-checker`
15. `figure-table-planner`
16. `solution-package-builder`
17. `modeling-figure-orchestrator` (route, render, inspect, and manifest data and non-data figures)
18. `academic-paper` (`plan` / `outline-only`, optional global outline and claim-evidence map)
19. `paper-section-writer` (canonical per-Qx drafting from solution packages)
20. **Modeler review → Programmer review**
21. `reference-manager` (verify citations are real)
22. `paper-polisher` (style / hedging / formula consistency within paper)
23. `academic-paper` (`abstract-only` / `revision`, optional whole-paper writing enhancement)
24. `academic-paper-reviewer` (optional read-only contest panel; advisory and cannot pass G6)
25. **Independent audit layer (Gate G6 — three orthogonal audits, all must PASS):**
...
26. Back to `workflow-orchestrator` for final assembly (only when Gate G6 passes)
