...
- `paper/figures/figure_manifest.json` — figure source, backend, output, claim, and QA metadata.
...
5.5. **Audit figure manifest alignment.**
   - Verify every promoted Type 3/4 figure in the paper has a manifest record, existing output,
     source artifact, and `qa_status: PASS`.
   - Verify the manifest's plotted fields and backend agree with the generator and source files.
   - Verify `target_width`, `target_height_cm`, `min_font_pt`, raster pixels/effective resolution,
     density strategy, and annotation statuses agree with the paper insertion and QA log.
