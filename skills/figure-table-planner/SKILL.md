...
**Requirements**:
- Must be publication-quality: clear labels, readable fonts, proper legends, vector output when
  supported, and adequate effective raster resolution at final insertion size. Follow an official
  DPI rule when present; otherwise do not treat 300 dpi as an unconditional gate.
...
1. Identify paper claims and evidence needs.
   ...
   - Build a required-coverage matrix for four evidence families: 核心模型原理 (core model
     principle), key result comparison, sensitivity/robustness analysis, and decision-scheme visualization. Each family must
     have a figure or table, or a written evidence-based reason why it is not applicable.
...
2. Inventory available artifacts.
   ...
   - Remove decorative artifacts and visuals that add no information beyond nearby text or tables.
...
4. Plan result figures (classify each).
   ...
   - Apply the 同类信息合并 principle: merge visuals with the same topic and comparison
     dimensions into one coherent multi-panel, grouped-comparison, or shared-axis figure.
   - Prefer multi-dimensional evidence in one figure. Use dual axes only when both units, scales,
     mappings, and the non-misleading reason for combining them are explicit.
...
7. Check visual evidence quality.
   ...
   - For every data figure, plan slots for key values, meaningful extrema, uncertainty/error bars or
     confidence intervals, and a comparison baseline. Each slot is `present` or `N/A: <reason>`;
     never invent uncertainty or a baseline merely to fill the slot.
   - For every principle/flow figure, plan core modules, parameter or data flow, decision exits, and
     key thresholds. Each omitted item needs an explicit reason.
   - Record target layout: single-column layouts use the full text width; spanning figures never
     exceed the total text block; paper figures have height >=5 cm; core-result figures normally use
     about one-third to one-half of a page when that improves legibility.
...
## 2.5 Visual Contract Fields

For every Type 3/4 figure, append these fields to the row or immediately below it:

```yaml
coverage_family: model_principle | key_comparison | sensitivity | decision_scheme | other
density_strategy: single_panel | multi_panel | grouped_comparison | shared_scale | justified_dual_axes
annotation_plan:
  key_values: present | "N/A: reason"
  extrema: present | "N/A: reason"
  uncertainty: present | "N/A: reason"
  baseline: present | "N/A: reason"
target_width: "1.00 text width" | "spanning <= text block"
target_height_cm: 5.0
```

`target_height_cm` must be at least `5.0`. The planner may use `N/A` only when the source artifact
cannot support the item and records the reason; it must never manufacture an error bar, interval,
baseline, or extremum.
...
- **Next skill**: `modeling-figure-orchestrator`
- **Routing note**: this planner defines figure need, type, source, placement, and human-owned `core_claim`; `modeling-figure-orchestrator` selects the renderer, runs visual QA, and writes the manifest before paper writing.
...
- Core model principle, key result comparison, sensitivity analysis, and decision-scheme
  visualization require corresponding visual evidence unless the plan records a defensible N/A.
- Merge same-theme, same-dimension information instead of splitting it into low-density figures.
- Every paper figure records `density_strategy`, `annotation_plan`, `target_width`, and
  `target_height`; target height is at least 5 cm.
...
- The four required evidence families are covered or carry a defensible N/A.
- No pair of paper figures should be merged under the 同类信息合并 principle.
- Annotation slots and final layout dimensions are complete; paper figures are at least 5 cm high.
