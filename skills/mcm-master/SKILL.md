...
## Codex plugin runtime preparation

Before first use in a modeling workspace, locate
`automcm-pro-runtime/bootstrap_runtime.py` under the plugin root and run:

```bash
python "<plugin-root>/automcm-pro-runtime/bootstrap_runtime.py" --target .
```

This prepares the required `scripts/`, `templates/`, and `AutoMCM_SOP.md` while preserving
existing files by default. Then run the `python scripts/...` commands below.
...
- Prefer vector output. For PNG, record pixels and effective resolution at final insertion size;
  follow official rules when stated, otherwise target about 240 effective dpi rather than treating
  300 dpi as a universal gate. Final scaled text must be >=7.5 pt (sixth-size/六号).
- Run `modeling-figure-orchestrator` before paper insertion so figure height, density, annotations,
  source reconciliation, and flowchart coverage pass the shared visual QA contract.
