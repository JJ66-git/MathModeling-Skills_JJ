...
description: "Use when a mathematical modeling paper needs an independent multi-perspective contest review, methodology-focused critique, adversarial challenge, revision roadmap, or re-review before the existing G6 audits."
metadata:
  version: "1.10.0"
...
Within `mathmodeling-skills`, read
`references/math-modeling-adaptation.md` before configuring reviewers or producing a decision. The
original multi-reviewer framework remains intact, but contest-specific evidence and Gate G6
boundaries in the adaptation are authoritative.
