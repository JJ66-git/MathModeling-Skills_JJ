...
5. **Output contract**: final paper width, height >=5 cm, format (SVG primary, PNG secondary),
   smallest scaled font >=7.5 pt, raster pixel dimensions, and effective resolution at insertion.
...
def render_check(fig, figure_path, min_fontsize_pt=7.5, overlap_tolerance_px=2):
    ...
    for ax in fig.get_axes():
        ...
        if ax.get_legend():
            ...
            if ax.get_legend().get_title():
                text_artists.append(ax.get_legend().get_title())
...
**Tuning guidance**:
- `min_fontsize_pt=7.5` is the hard paper floor (Chinese sixth-size/六号) after final scaling;
  `min_fontsize_pt=9` is the slide floor.
- Dense heatmaps and compact annotations do not receive a smaller-font exemption. Increase the
  figure, merge labels, abbreviate with a defined legend, or move detail to an appendix.
...
- **Secondary is PNG** for preview and raster insertion. Record pixel dimensions and effective DPI
  at the final paper size. Follow an official DPI rule when present; otherwise target >=240
  effective dpi for bitmap insertion. `300 dpi` remains a sensible export default, not a universal
...
- [ ] Same-theme information is merged into one coherent comparison where possible; no decorative or low-information panel remains.
...
- [ ] Key values, meaningful extrema, uncertainty/error bars or confidence intervals, and baseline
      are visibly marked or recorded as `N/A: <reason>` without fabricated evidence.
- [ ] Final insertion width is within the text block, height is >=5 cm, and core-result figures use
      roughly one-third to one-half of a page when needed for readability.
- [ ] Raster pixel dimensions and effective DPI at insertion are recorded; target-width preview is sharp.
...
- [ ] **render_check passed** — `render_check_and_log(fig, path)` returned True; entry written to `paper/figures/render_check.log`. No final scaled font < 7.5pt; no text out of canvas; no text overlap; no missing axis labels.
...
- Let `modeling-figure-orchestrator` choose the backend. Use Python/matplotlib by default; use MATLAB only when its MCP escalation criteria are met.
- For a MATLAB route, follow `modeling-figure-orchestrator/references/matlab-mcp-plotting.md` and preserve the same claim-first, panel, color, source, and visual-QA rules.
