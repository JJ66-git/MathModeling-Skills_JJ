...
def draw_taylor_grid(ax: plt.Axes, ref_std: float = 1.0, rmax: float = 1.75) -> None:
    ...
    ax.set_xticks(ticks)
    ax.set_yticks(ticks)
    ax.set_xticklabels([f"{t:.2f}" if t else "0" for t in ticks], fontsize=7.5)
    ax.set_yticklabels([f"{t:.2f}" if t else "0" for t in ticks], fontsize=7.5)
    ...
    for corr in corr_ticks:
        ...
        ax.text(x, y, label, fontsize=7.5, ha="center", va="center", rotation=angle, rotation_mode="anchor")
    ...
    ax.text(ref_std, -0.060, "Observed", fontsize=7.5, ha="center", va="top")
