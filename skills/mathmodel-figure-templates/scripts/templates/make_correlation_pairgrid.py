...
def draw_scatter_cell(ax: plt.Axes, x: np.ndarray, y: np.ndarray, xlabel: str, ylabel: str) -> None:
    ...
    ax.set_xlim(-3.1, 3.1)
    ax.set_ylim(-3.1, 3.1)
    ax.set_xlabel(xlabel, fontsize=7.5, labelpad=1)
    ax.set_ylabel(ylabel, fontsize=7.5, labelpad=1)
...
def draw_hist_cell(ax: plt.Axes, values: np.ndarray, xlabel: str) -> None:
    ...
    ax.set_xlabel(xlabel, fontsize=7.5, labelpad=1)
    ax.set_ylabel("Count", fontsize=7.5, labelpad=1)
...
def draw_corr_cell(
    ...
    ax.text(0.5, 0.46, f"{r:.2f}", ha="center", va="center", fontsize=7.5, color=text_color, transform=ax.transAxes)
    star_text = stars_for_p(p_value)
    if star_text:
        ax.text(0.5, 0.68, star_text, ha="center", va="center", fontsize=7.5, fontweight="bold", color=text_color, transform=ax.transAxes)
...
def make_figure(output_stem: Path) -> None:
    ...
    cbar.ax.tick_params(labelsize=7.5, width=0.45, length=2)
