...
def draw_lower_corr(ax: plt.Axes, corr: np.ndarray) -> None:
    ...
    ax.set_xticklabels([item.name for item in FEATURES], rotation=90, fontsize=7.5, fontweight="bold")
...
def draw_split_violin(
    ...
    if spec.log_scale:
        ax.set_yscale("log")
    ax.set_xticks([])
    ax.set_ylabel(spec.label, fontsize=7.5, fontweight="bold", labelpad=1)
    ax.tick_params(axis="y", labelsize=7.5, length=2, width=0.6, pad=1)
    for spine in ax.spines.values():
